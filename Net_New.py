...
class BilinearPooling(nn.Module):
    def __init__(self, cout=2,  dout=256):
        ...
        self.pool2 = nn.Sequential(
            nn.Conv2d(256, dout, kernel_size=1, stride=1, padding=0, bias=False),
            # nn.BatchNorm2d(cout),
            nn.ReLU(),
        )
    ...
    def forward(self, Fd, Fc):
        Fc0 = self.pool1(Fc)  # Content
        Fd0 = self.pool2(Fd)
        out = self.bilinear_pooling(Fc0, Fd0)
        return out
...
class RegNetnewV3(nn.Module):
    def __init__(self, cout=256, dout=256):
        super(RegNetnewV3, self).__init__()

        self.cout = cout

        self.bipool = BilinearPooling(cout=2, dout=((256+cout) // 2))

        self.poolc = nn.Sequential(
            # nn.Conv2d(512, cout, kernel_size=3, stride=1, padding=1, bias=True),
            # nn.BatchNorm2d(cout),
            # nn.ReLU(),
    ...
    def forward(self, fd, Fdmaps, Fc):
        Fd1, Fd2, Fd3, Fd4 = Fdmaps
        # Fd1 = nn.functional.interpolate(Fd1, (Fc.size(2), Fc.size(3)), mode='bilinear')
        Fd2 = nn.functional.interpolate(Fd2, (Fd1.size(2), Fd1.size(3)), mode='bilinear')
        Fd3 = nn.functional.interpolate(Fd3, (Fd1.size(2), Fd1.size(3)), mode='bilinear')
        Fd4 = nn.functional.interpolate(Fd4, (Fd1.size(2), Fd1.size(3)), mode='bilinear')
        Fc = nn.functional.interpolate(Fc, (Fd1.size(2), Fd1.size(3)), mode='bilinear')
        Fd = torch.cat([Fd1, Fd2, Fd3, Fd4], dim=1)
        fcd = self.bipool(Fd, Fc)
        # fcd = nn.functional.softmax(torch.sigmoid(fcd.view(fcd.size(0), -1)), dim=-1)
        fd = nn.functional.normalize(fd)  # L2-Norm
        fc = self.poolc(Fc)
        fc = nn.functional.normalize(fc)  # L2-Norm
        feat = torch.cat([fc.view(fc.size(0), -1), fd], dim=-1)
        feat = torch.mul(feat, fcd) + feat
        # print([fc.shape, fd.shape, fi.shape])
        out = self.fc(feat)
        return out


class VISORNetV3(nn.Module):
    def __init__(self, cout=16, dout=16):
        super(VISORNetV3, self).__init__()

        self.CEnc = nn.DataParallel(BackboneRes(layer=34))
        # self.CEnc = nn.DataParallel(Cont_Enc())
        self.DEnc = nn.DataParallel(Diff_Enc())
        self.Reg = nn.DataParallel(RegNetnewV3(cout=cout, dout=dout))
        # self.CEnc = Cont_Enc() # for calculate FLOPs
        # self.DEnc = Diff_Enc()
        # self.Reg = RegNetnewV3(cout=cout, dout=dout)
        self.fc = nn.Sequential(
            nn.Linear((256+cout), 256),
            # nn.BatchNorm1d(256),
            nn.ReLU(True),
    ...
    def forward(self, img):
        ...
        featd, _, maps, _ = self.DEnc(img)

        # for i in range(len(maps)):
        #     maps[i] = maps[i].detach()

        score = self.Reg(featd, maps, featc.detach())

        return score


class RegNetLong(nn.Module):
    def __init__(self, cout=16, dout=16):
        super(RegNetLong, self).__init__()
    ...
    def forward(self, fd, Fc):
        fd = nn.functional.normalize(fd)  # L2-Norm
        fc = self.pool1(Fc)  # Samentic
        fc = nn.functional.normalize(fc.view(fc.size(0), -1))  # L2-Norm
        feat = torch.cat([fc, fd], dim=-1)
        out = self.fc(feat)
        return out
